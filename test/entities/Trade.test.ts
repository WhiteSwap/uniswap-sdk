...
describe('Trade', () => {
  ...
  const token1 = new Token(1, '0x0000000000000000000000000000000000000002', 18, 't1', 't1')
  const token2 = new Token(1, '0x0000000000000000000000000000000000000003', 18, 't2', 't2')
  const token3 = new Token(1, '0x0000000000000000000000000000000000000004', 18, 't3', 't3')
  ...
  const pair_weth_0 = new Pair(
    CurrencyAmount.fromRawAmount(WRAPPED_NATIVE_CURRENCY[ChainId.MAINNET], JSBI.BigInt(1000)),
    CurrencyAmount.fromRawAmount(token0, JSBI.BigInt(1000))
  )
  ...
  it('can be constructed with ETHER as input', () => {
    const trade = new Trade(
      new Route([pair_weth_0], NATIVE_CURRENCY[pair_weth_0.chainId], token0),
      CurrencyAmount.fromRawAmount(NATIVE_CURRENCY[pair_weth_0.chainId], JSBI.BigInt(100)),
      TradeType.EXACT_INPUT
    )
    expect(trade.inputAmount.currency).toEqual(NATIVE_CURRENCY[pair_weth_0.chainId])
  ...
  it('can be constructed with ETHER as output', () => {
    const trade = new Trade(
      new Route([pair_weth_0], token0, NATIVE_CURRENCY[pair_weth_0.chainId]),
      CurrencyAmount.fromRawAmount(NATIVE_CURRENCY[pair_weth_0.chainId], JSBI.BigInt(100)),
      TradeType.EXACT_OUTPUT
    )
    expect(trade.inputAmount.currency).toEqual(token0)
    expect(trade.outputAmount.currency).toEqual(NATIVE_CURRENCY[pair_weth_0.chainId])
  })
  ...
  describe('#bestTradeExactIn', () => {
    it('throws with empty pairs', () => {
      expect(() => Trade.bestTradeExactIn([], CurrencyAmount.fromRawAmount(token0, JSBI.BigInt(100)), token2)).toThrow('PAIRS')
    })
    ...
    it('provides best route', () => {
      const result = Trade.bestTradeExactIn([pair_0_1, pair_0_2, pair_1_2], CurrencyAmount.fromRawAmount(token0, JSBI.BigInt(100)), token2)
    ...
    it('doesnt throw for zero liquidity pairs', () => {
      expect(Trade.bestTradeExactIn([empty_pair_0_1], CurrencyAmount.fromRawAmount(token0, JSBI.BigInt(100)), token1)).toHaveLength(0)
    })

    it('respects maxHops', () => {
      const result = Trade.bestTradeExactIn([pair_0_1, pair_0_2, pair_1_2], CurrencyAmount.fromRawAmount(token0, JSBI.BigInt(10)), token2, {
        maxHops: 1
      })
    ...
    it('insufficient input for one pair', () => {
      const result = Trade.bestTradeExactIn([pair_0_1, pair_0_2, pair_1_2], CurrencyAmount.fromRawAmount(token0, JSBI.BigInt(1)), token2)
      expect(result).toHaveLength(1)
      expect(result[0].route.pairs).toHaveLength(1) // 0 -> 2 at 10:11
      expect(result[0].route.path).toEqual([token0, token2])
    ...
    it('no path', () => {
      const result = Trade.bestTradeExactIn([pair_0_1, pair_0_3, pair_1_3], CurrencyAmount.fromRawAmount(token0, JSBI.BigInt(10)), token2)
    ...
    it('works for ETHER currency input', () => {
      ...
      expect(result).toHaveLength(2)
      expect(result[0].inputAmount.currency).toEqual(NATIVE_CURRENCY[pair_weth_0.chainId])
      expect(result[0].route.path).toEqual([WRAPPED_NATIVE_CURRENCY[ChainId.MAINNET], token0, token1, token3])
      expect(result[0].outputAmount.currency).toEqual(token3)
      expect(result[1].inputAmount.currency).toEqual(NATIVE_CURRENCY[pair_weth_0.chainId])
      expect(result[1].route.path).toEqual([WRAPPED_NATIVE_CURRENCY[ChainId.MAINNET], token0, token3])
    ...
    it('works for ETHER currency output', () => {
      const result = Trade.bestTradeExactIn(
        ...
        NATIVE_CURRENCY[pair_weth_0.chainId]
      )
      expect(result).toHaveLength(2)
      expect(result[0].inputAmount.currency).toEqual(token3)
      expect(result[0].route.path).toEqual([token3, token0, WRAPPED_NATIVE_CURRENCY[ChainId.MAINNET]])
      expect(result[0].outputAmount.currency).toEqual(NATIVE_CURRENCY[pair_weth_0.chainId])
      expect(result[1].inputAmount.currency).toEqual(token3)
      expect(result[1].route.path).toEqual([token3, token1, token0, WRAPPED_NATIVE_CURRENCY[ChainId.MAINNET]])
      expect(result[1].outputAmount.currency).toEqual(NATIVE_CURRENCY[pair_weth_0.chainId])
    })
  })
  ...
  describe('#bestTradeExactOut', () => {
    it('throws with empty pairs', () => {
      expect(() => Trade.bestTradeExactOut([], token0, CurrencyAmount.fromRawAmount(token2, JSBI.BigInt(100)))).toThrow('PAIRS')
    })
    it('throws with max hops of 0', () => {
      expect(() =>
    ...
    it('provides best route', () => {
      const result = Trade.bestTradeExactOut([pair_0_1, pair_0_2, pair_1_2], token0, CurrencyAmount.fromRawAmount(token2, JSBI.BigInt(100)))
      expect(result).toHaveLength(2)
      expect(result[0].route.pairs).toHaveLength(1) // 0 -> 2 at 10:11
      expect(result[0].route.path).toEqual([token0, token2])
    ...
    it('doesnt throw for zero liquidity pairs', () => {
      expect(Trade.bestTradeExactOut([empty_pair_0_1], token1, CurrencyAmount.fromRawAmount(token1, JSBI.BigInt(100)))).toHaveLength(0)
    })
    ...
    it('no path', () => {
      const result = Trade.bestTradeExactOut([pair_0_1, pair_0_3, pair_1_3], token0, CurrencyAmount.fromRawAmount(token2, JSBI.BigInt(10)))
      expect(result).toHaveLength(0)
    })
