import { Token, WETH, ChainId, Pair, TokenAmount, Route, NATIVE_CURRENCY } from '../src'
...
describe('Route', () => {
  ...
  it('supports ether input', () => {
    const route = new Route([pair_0_weth], NATIVE_CURRENCY[pair_0_weth.chainId])
    expect(route.pairs).toEqual([pair_0_weth])
    expect(route.input).toEqual(NATIVE_CURRENCY[pair_0_weth.chainId])
    expect(route.output).toEqual(token0)
  })

  it('supports ether output', () => {
    const route = new Route([pair_0_weth], token0, NATIVE_CURRENCY[pair_0_weth.chainId])
    expect(route.pairs).toEqual([pair_0_weth])
    expect(route.input).toEqual(token0)
    expect(route.output).toEqual(NATIVE_CURRENCY[pair_0_weth.chainId])
  })
})
