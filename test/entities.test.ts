...
function decimalize(amount: BigintIsh, decimals: number): JSBI {
  return JSBI.multiply(JSBI.BigInt(amount), JSBI.exponentiate(JSBI.BigInt(10), JSBI.BigInt(decimals)))
}
const WETH = WRAPPED_NATIVE_CURRENCY[CHAIN_ID]
...
describe('entities', () => {
  DECIMAL_PERMUTATIONS.forEach(decimals => {
    describe(`decimals permutation: ${decimals}`, () => {
      let tokens: Token[]
      beforeAll(() => {
        tokens = ADDRESSES.map((address, i) => new Token(CHAIN_ID, address, decimals[i], `test${i}`, `test${i}`))
      })
